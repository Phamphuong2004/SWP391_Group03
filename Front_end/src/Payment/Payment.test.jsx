...
describe('Payment', () => {
  ...
  it('shows no appointment message when no appointment data', () => {
    ...
    expect(screen.getByText('Không tìm thấy thông tin lịch hẹn')).toBeInTheDocument()
  ...
  it('handles API fetch error', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith('Không tìm thấy thông tin lịch hẹn')
    })
  })
  ...
  it('shows QR section when online payment is selected', async () => {
    ...
    expect(screen.getByText('Vui lòng quét mã QR bằng app ngân hàng hoặc ví điện tử để thanh toán.')).toBeInTheDocument()
  })
  ...
  it('warns when trying to pay without selecting payment method', async () => {
    ...
    expect(mockToast.warn).toHaveBeenCalledWith('Vui lòng chọn phương thức thanh toán.')
  })
  ...
  it('processes cash payment successfully', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.success).toHaveBeenCalledWith('Thanh toán thành công! Lịch hẹn của bạn đã được xác nhận.')
    })
  })
  ...
  it('handles payment failure', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith('Thanh toán thất bại. Payment failed')
    })
  })
  ...
  it('handles payment failure without response message', async () => {
    ...
    await waitFor(() => {
      expect(mockToast.error).toHaveBeenCalledWith('Thanh toán thất bại. Network error')
    })
  })
  ...
  it('handles missing service details gracefully', () => {
    ...
    render(
      <PaymentWrapper initialState={{ appointment: appointmentWithUnknownService }}>
        <Payment />
      </PaymentWrapper>
    )
