...
export default function ServiceTestPurposeStaff() {
  ...
  const filteredServices = data.filter(service =>
    service.name && service.name.toLowerCase().includes(serviceSearch.toLowerCase())
  );
  ...
  const handleAddPurpose = () => {
    if (!selectedService || !newPurpose.trim()) return;
  ...
  const handleEditPurpose = () => {
    if (!editPurpose.value.trim() || editPurpose.idx === null) return;
    setData(prev => prev.map(s =>
      s.id === selectedService.id ? {
        ...s,
        testPurposes: s.testPurposes.map((tp, idx) => idx === editPurpose.idx ? editPurpose.value.trim() : tp)
      } : s
    ));
      ...
      <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 24 }}>
        <thead>
          <tr style={{ background: "#f0f0f0" }}>
            <th style={{ border: "1px solid #1976d2", padding: 8 }}>Tên dịch vụ</th>
            <th style={{ border: "1px solid #1976d2", padding: 8 }}>Số lượng Test Purpose</th>
            <th style={{ border: "1px solid #1976d2", padding: 8 }}>Hành động</th>
          </tr>
        </thead>
        <tbody>
          {filteredServices.map(service => (
            <tr key={service.id} style={{ background: selectedService && selectedService.id === service.id ? "#e3f2fd" : "white" }}>
              <td style={{ border: "1px solid #1976d2", padding: 8 }}>{service.name}</td>
              <td style={{ border: "1px solid #1976d2", padding: 8 }}>{service.testPurposes.length}</td>
          ...
          {filteredServices.length === 0 && (
            <tr><td colSpan={3} style={{ textAlign: "center", padding: 16 }}>Không tìm thấy dịch vụ</td></tr>
          )}
        </tbody>
      </table>
      {selectedService && (
        <div style={{ border: "1px solid #1976d2", borderRadius: 8, padding: 18, marginTop: 12 }}>
                ...
                {editPurpose.idx === idx ? (
                  <>
                    <input
                      ...
                      onChange={e => setEditPurpose({ ...editPurpose, value: e.target.value })}
                  ...
                  <>
                    {tp}
                    <button style={{ marginLeft: 8 }} onClick={() => setEditPurpose({ idx, value: tp })}>Sửa</button>
                  </>
                )}
              </li>
            ))}
            {getFilteredPurposes(selectedService).length === 0 && (
              <li style={{ color: '#888' }}>Không có test purpose nào phù hợp</li>
            )}
          </ul>
